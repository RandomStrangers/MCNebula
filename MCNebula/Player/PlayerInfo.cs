...
namespace MCNebula 
{
    public static class PlayerInfo 
    {
        ...
        public static int UniqueIPCount()
        {
            Player[] players = Online.Items;
            Dictionary<string, bool> uniqueIPs2 = new Dictionary<string, bool>();

            foreach (Player p in players)
            {
                uniqueIPs2[p.ip] = true;
            }
            return uniqueIPs2.Count;
        }
        public static int NonHiddenCount()
        {
            Player[] players = Online.Items;
            Dictionary<string, bool> NonHidden = new Dictionary<string, bool>();

            foreach (Player p in players)
            {
                if (!p.hidden) NonHidden[p.truename] = true;
            }
            return NonHidden.Count;
        }
        public static int Count()
        {
            Player[] players = Online.Items;
            Dictionary<string, bool> LoggedIn = new Dictionary<string, bool>();

            foreach (Player p in players)
            {
                if (!p.loggedIn) LoggedIn[p.truename] = true;
            }
