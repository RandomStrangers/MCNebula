...
namespace MCNebula.Commands.Maintenance {
    public sealed class CmdServer : Command2 {
        ...
        static void DoRestore(Player p) {
            if (!CheckPerms(p)) {
                p.Message("Only Console or the Server Owner can restore the server."); return;
            }
            Backup.Extract(p);
        }
        ...
        static bool CheckPerms(Player p) {
            ...
            if (Server.Config.OwnerName.CaselessEq("The owner")) return false;
