...
namespace MCNebula.Network
{
    ...
    public sealed class ClassiCubeBeat2 : Heartbeat
    {
        string proxyUrl;
        ...
        static void OnSuccess(string text) {
            text = Truncate(text);
            Logger.Log(LogType.SystemActivity, text);
            File.WriteAllText("text/externalurl2.txt", text);
            Logger.Log(LogType.SystemActivity, "Server URL found: " + text);
        }
        
        static void OnError(string error) {
            error = Truncate(error);
            Logger.Log(LogType.SystemActivity, error);
