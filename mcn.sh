...
mcndir="./"							##  Folder containing MCNebula.exe
...
gamename="RedCraft - MCNebula"		##  Arbitrary name of server, will not affect actual server name
gamepid="${mcndir}/mcn.pid"			##  If you do not know what this is, do not worry about it, for "silent" mode only.
gamelog="${mcndir}/mcn.log"			##  This logs everything sent to console, if started in "silent" mode
...
if [ ! -f "${mcndir}/MCNebula.exe" ]; then
	echo -e "cannot find ${mcndir}/MCNebula.exe!"
...
case "$1" in
	silent)
		...
		if ps -ef |grep "${monobin} ${gameopt} ${mcndir}/MCNebula.exe"|awk -F" " -v game=${monobin} 'BEGIN {status=1} ; $8 == game {status=0} ; END {exit status}' ; then
			...
			if ${monobin} ${gameopt} ${mcndir}/MCNebula.exe 1>> ${gamelog} 2>&1 & sleep 3 ; then
				pid=`ps -ef |grep "${monobin} ${gameopt} ${mcndir}/MCNebula.exe"|awk -F" " -v game=${monobin} 'BEGIN {status=1} ; $8 == game { print $2 } ; END {exit status}'`
				echo ${pid} > ${gamepid}
...
;;
	normal)
		...
		if ps -ef |grep "${monobin} ${gameopt} ${mcndir}/MCNebula.exe"|awk -F" " -v game=${monobin} 'BEGIN {status=1} ; $8 == game {status=0} ; END {exit status}' ; then
			...
			echo -e
			${monobin} ${gameopt} ${mcndir}/MCNebula.exe
			if $autorestart ; then
				$0
			else
				exit 0
...
;;
	*)
		if [ -f "${mcndir}/MCNebula_.update" ]; then
			if [ -f "${mcndir}/MCNebula.update" ]; then
				echo
				echo Update Found!
				echo -n Applying update:
				rm ${mcndir}/MCNebula.exe
				rm ${mcndir}/MCNebula_.dll
				mv ${mcndir}/MCNebula.update ${mcndir}/MCNebula.exe
				mv ${mcndir}/MCNebula_.update ${mcndir}/MCNebula_.dll
				if [ -f "${mcndir}/MCNebula_.update" ]; then
					if [ -f "${mcndir}/MCNebula.update" ]; then
						echo -e FAILED!
						if [ -f "${mcndir}/MCNebula_.dll" ]; then
							if [ -f "${mcndir}/MCNebula.exe" ]; then
								$0 ${default}
							fi
						else
